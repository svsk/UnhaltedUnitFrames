...
function UUF:GetPowerBarAnchorAndSize(unitFrame, FrameDB, PowerBarDB)
    local anchorPoint = {
        Position = "BOTTOMLEFT",
        OffsetX = 1,
        OffsetY = 1
    ...
    local size = {
        Width = FrameDB.Width - 2, -- unitFrame:GetWidth() - 2 -- when updating
        Height = PowerBarDB.Height
    }

    if FrameDB.Orientation == "VERTICAL" then
        anchorPoint.Position = "BOTTOMRIGHT"
        anchorPoint.OffsetX = -1
        anchorPoint.OffsetY = 1

        size.Width = PowerBarDB.Height
        size.Height = FrameDB.Height -- unitFrame:GetHeight() when updating
    end
...
function UUF:CreateUnitPowerBar(unitFrame, unit)
    ...
    local anchorPoint, size = UUF:GetPowerBarAnchorAndSize(unitFrame, FrameDB, PowerBarDB)

    local PowerBar = CreateFrame("StatusBar", UUF:FetchFrameName(unit) .. "_PowerBar", unitContainer)
    PowerBar:SetPoint(anchorPoint.Position, unitContainer, anchorPoint.Position, anchorPoint.OffsetX, anchorPoint
        .OffsetY)
    PowerBar:SetSize(size.Width, size.Height)
    PowerBar:SetStatusBarTexture(UUF.Media.Foreground)
    PowerBar:SetStatusBarColor(PowerBarDB.Foreground[1], PowerBarDB.Foreground[2], PowerBarDB.Foreground[3],
        PowerBarDB.Foreground[4] or 1)
    PowerBar:SetFrameLevel(unitContainer:GetFrameLevel() + 2)
    PowerBar:SetOrientation(FrameDB.Orientation)
    ...
    PowerBar.Background:SetPoint(anchorPoint.Position, unitContainer, anchorPoint.Position, anchorPoint.OffsetX,
        anchorPoint.OffsetY)
    PowerBar.Background:SetSize(size.Width, size.Height)
    PowerBar.Background:SetTexture(UUF.Media.Background)
    PowerBar.Background:SetVertexColor(PowerBarDB.Background[1], PowerBarDB.Background[2], PowerBarDB.Background[3],
...
function UUF:UpdateUnitPowerBar(unitFrame, unit)
    ...
    if PowerBarDB.Enabled then
        ...
        if unitFrame.Power then
            ...
            unitFrame.Power:SetPoint(anchorPoint.Position, unitFrame.Container, anchorPoint.Position, anchorPoint
                .OffsetX, anchorPoint.OffsetY)
            unitFrame.Power:SetSize(size.Width, size.Height)
            unitFrame.Power:SetStatusBarColor(PowerBarDB.Foreground[1], PowerBarDB.Foreground[2],
                PowerBarDB.Foreground[3], PowerBarDB.Foreground[4] or 1)
            unitFrame.Power:SetStatusBarTexture(UUF.Media.Foreground)
            unitFrame.Power:SetOrientation(FrameDB.Orientation)
        ...
        if unitFrame.Power.Background then
            ...
            unitFrame.Power.Background:SetSize(size.Width, size.Height)
            unitFrame.Power.Background:SetVertexColor(PowerBarDB.Background[1], PowerBarDB.Background[2],
                PowerBarDB.Background[3], PowerBarDB.Background[4] or 1)
